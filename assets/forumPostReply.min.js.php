...
l),e.querySelector(".bearcms-forum-post-page-send-button").style.removeProperty("display")):(d.setAttribute("readonly",!0),d.style.cursor="pointer",d.addEventListener("click",l),e.querySelector(".bearcms-forum-post-page-send-button").style.display="none")};null!==b?c(b):clientPackages.get("users").then(function(f){f.currentUser.exists().then(function(e){c(e)})})},l=function(a){var b=a.target.closest("form").id;clientPackages.get("users").then(function(c){k(b);c.openLogin()})},p=function(a){clientPackages.get("-bearcms-forums-html5domdocument").then(function(b){var c=
